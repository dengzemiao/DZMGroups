...
@implementation DZMGroupsToolBar
...
- (void)selectIndex:(NSInteger)index animated:(BOOL)animated {
    ...
    if (animated) {
        ...
        __weak DZMGroupsToolBar *weakSelf = self;
    ...
    if ([self.delegate respondsToSelector:@selector(toolBar:scrollIndex:)]) {
        ...
        [self.delegate toolBar:self scrollIndex:index];
    }
}
...
- (void)clickItem:(UIButton *)item {
    ...
    [self selectIndex:item.tag animated:YES];
    
    if ([self.delegate respondsToSelector:@selector(toolBar:clickIndex:)]) {
        
        [self.delegate toolBar:self clickIndex:item.tag];
    }
}

- (void)dealloc {
    
    NSLog(@"DZMGroupsToolBar - 释放了");
}
